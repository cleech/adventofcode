...
fn race(input: &str, seconds: usize) -> usize {
    input.lines()
         .filter_map(|line| line.parse::<Reindeer>().ok())
...
mod test {
    ...
    const EXAMPLE_DATA: [&'static str; 2] = ["Comet can fly 14 km/s for 10 seconds, but then \
                                              must rest for 127 seconds.",
                                             "Dancer can fly 16 km/s for 11 seconds, but then \
                                              must rest for 162 seconds."];
