I made good use of some of the functionality of the `itertools` crate on this
one, particularly `Unfold` and `Batching`. While a lot of solutions out there
only aimed to get the needed answers about string length, I decided to actually
fully implement the string escaping/un-escaping.

An older version assumed some unspecified 8-bit encoding, as the two char hex
escapes match up with mapping those down to 7-bit clean ASCII. But Rust uses a
multi-byte UTF-8 encoding for `String` types, so I did later update the code to
deal in `char` (and `u32`) and not `u8`.
